...
private fun SettingsV19Screen(
    ...
    BoxWithConstraints(Modifier.fillMaxSize()) {
        val compact = maxWidth < 380.dp || systemFontScale >= 1.20f
        val columns = if (compact) 1 else if (maxWidth < 650.dp) 2 else 3
        ...
        Column(
            ...
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Column(Modifier.weight(1f)) {
                    ...
                    Text(tr(lang,"Todos los cambios se guardan automáticamente y permanecen al cerrar la app.","All changes save automatically and remain after closing the app."),style=MaterialTheme.typography.bodyMedium)
                }
            }

            SettingsCardV19(tr(lang,"Idioma y tratamiento","Language and title")) {
            ...
            SettingsCardV19(tr(lang,"Paleta de aves","Bird palette")) {
            ...
            SettingsCardV19(tr(lang,"Tipo de letra","Typeface")) {
            ...
            SettingsCardV19(tr(lang,"Tamaño de letra","Text size")) {
                ...
                Card(colors=CardDefaults.cardColors(containerColor=MaterialTheme.colorScheme.primaryContainer),modifier=Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(12.dp),verticalArrangement=Arrangement.spacedBy(5.dp)) {
                        ...
                        Text(tr(lang,"También se respeta la escala de letra configurada en Android.","Android system font scaling is also respected."),style=MaterialTheme.typography.bodyMedium)
                    }
                }
            }

            SettingsCardV19(tr(lang,"Adaptación detectada","Detected adaptation")) {
                Text("📱 ${tr(lang,"Ancho disponible","Available width")}: ${maxWidth.value.toInt()} dp")
                Text("🔤 ${tr(lang,"Escala de letra Android","Android font scale")}: ${"%.0f".format(systemFontScale*100)}%")
                Text(if(compact) tr(lang,"Modo compacto: una columna cuando conviene para evitar recortes.","Compact mode: one column where needed to prevent clipping.") else tr(lang,"Modo estándar/ampliado: se aprovecha el ancho disponible.","Standard/expanded mode: available width is used."))
            }
...
@Composable
private fun SettingsCardV19(title:String,content:@Composable Column.()->Unit) {
    ...
    Card(onClick=onClick,modifier=modifier,colors=CardDefaults.cardColors(containerColor=if(selected)MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.surface),border=BorderStroke(if(selected)2.dp else 1.dp,if(selected)MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.outline.copy(alpha=.4f)),shape=RoundedCornerShape(14.dp)) {
        Column(Modifier.fillMaxWidth().padding(9.dp),horizontalAlignment=Alignment.CenterHorizontally,verticalArrangement=Arrangement.spacedBy(6.dp)) {
            ...
            Row(horizontalArrangement=Arrangement.spacedBy(4.dp)) {
                paletteSwatches(style).forEach { c -> Box(Modifier.size(16.dp).background(c,CircleShape)) }
            }
