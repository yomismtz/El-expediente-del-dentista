...
fun EndodonticInteractiveV2Screen(
    ...
    fun changeDentition(id: String) {
        dentition = id
        val newTooth = if (id == "primary") {
            selectedTooth.takeIf { it in ClinicalContent.primaryTeeth } ?: 75
        } else {
            selectedTooth.takeIf { it in ClinicalContent.permanentTeeth } ?: 36
        }
        changeTooth(newTooth)
    ...
    LazyColumn(
        ...
        when (tab) {
            0 -> {
                ...
                item {
                    NoticeCard(
                        tr(
                            lang,
                            "La AAE y la ESE mantienen en revisión una actualización de terminología diagnóstica. Esta app conserva las categorías docentes vigentes del expediente y las presenta como orientación, no como diagnóstico automático definitivo.",
                            "AAE and ESE are reviewing updated diagnostic terminology. This app retains the record's teaching categories and presents them as guidance, not as an automatic definitive diagnosis."
                        )
                    )
                }
                item {
                    SectionCard(tr(lang, "4 · Orientación terapéutica", "4 · Treatment orientation")) {
                        ...
                        procedureOptions(dentition, lang).forEach { (id, label) ->
                            FilterChip(
                                selected = procedure == id,
                                onClick = { procedure = id; completed.clear() },
                                label = { Text(label) },
            ...
            3 -> {
                ...
                item { PediatricEndoCard(tr(lang, "Diente temporal · pulpa vital", "Primary tooth · vital pulp"), tr(lang, "Con pulpa normal o pulpitis reversible pueden considerarse terapias pulpares vitales según profundidad, exposición y hemostasia: tratamiento pulpar indirecto, recubrimiento selectivo o pulpotomía cuando esté indicada. Una pulpotomía completa también puede considerarse en casos seleccionados con signos de pulpitis irreversible si no hay signos clínicos/radiográficos de infección, bajo criterios actuales y supervisión.", "With normal pulp or reversible pulpitis, vital pulp therapies may be considered according to depth, exposure and hemostasis: indirect pulp treatment, selected capping or pulpotomy when indicated. Complete pulpotomy may also be considered in selected cases showing irreversible-pulpitis signs when clinical/radiographic infection is absent, under current criteria and supervision.")) }
                item { PediatricEndoCard(tr(lang, "Diente temporal · pulpa no vital", "Primary tooth · non-vital pulp"), tr(lang, "En pulpitis irreversible no recuperable o necrosis, la pulpectomía puede ser una opción si el diente es restaurable y conservarlo es útil. La reabsorción fisiológica, anatomía de conductos, proximidad al sucesor permanente y material reabsorbible condicionan la técnica y el pronóstico.", "For non-recoverable irreversible pulpitis or necrosis, pulpectomy may be an option when the tooth is restorable and worth retaining. Physiologic resorption, canal anatomy, the permanent successor and resorbable filling material affect technique and prognosis.")) }
                item { PediatricEndoCard(tr(lang, "Permanente joven · pulpa vital", "Immature permanent · vital pulp"), tr(lang, "La prioridad es conservar tejido pulpar vital cuando sea razonable para permitir desarrollo radicular. Dependiendo del diagnóstico y la exposición pueden considerarse tratamiento pulpar indirecto, recubrimiento directo, pulpotomía parcial o completa con biomaterial indicado.", "The priority is to preserve vital pulp when reasonable so root development can continue. Depending on diagnosis and exposure, indirect pulp treatment, direct pulp capping, partial pulpotomy or complete pulpotomy with an indicated biomaterial may be considered.")) }
                item { PediatricEndoCard(tr(lang, "Permanente joven · necrosis", "Immature permanent · necrosis"), tr(lang, "La raíz incompleta cambia el plan. Puede requerir endodoncia regenerativa, apicoformación u otra estrategia especializada. No debe aplicarse automáticamente el protocolo de un diente permanente maduro.", "An incomplete root changes the plan. Regenerative endodontics, apexification or another specialized strategy may be needed. A mature permanent-tooth protocol should not be applied automatically.")) }
                item {
                    NoticeCard(
                        tr(
                            lang,
            ...
            4 -> {
                ...
                item {
                    SectionCard(tr(lang, "Hipoclorito de sodio · uso seguro", "Sodium hypochlorite · safe use")) {
                        Text(tr(lang, "El NaOCl se usa ampliamente por su acción antimicrobiana y capacidad de disolver tejido orgánico. La concentración debe seguir el protocolo institucional y el producto dental; concentraciones mayores aumentan eficacia de disolución, pero también toxicidad si ocurre extrusión.", "NaOCl is widely used for antimicrobial action and organic-tissue dissolution. Concentration should follow the institution and dental product; higher concentrations can increase tissue dissolution but also toxicity if extrusion occurs."))
                        Text("• ${tr(lang, "Aislamiento absoluto según el protocolo clínico.", "Rubber-dam isolation according to clinical protocol.")}")
                ...
                item {
                    SectionCard(tr(lang, "EDTA y otros irrigantes", "EDTA and other irrigants")) {
                        Text(tr(lang, "EDTA se utiliza como quelante para componente inorgánico/capa de barrillo según el protocolo. NaOCl y EDTA deben manejarse como pasos separados porque interactúan y pueden modificar sus propiedades.", "EDTA is used as a chelator for inorganic components/smear layer according to protocol. NaOCl and EDTA should be handled as separate steps because they interact and can alter their properties."))
                        Text(tr(lang, "Evita mezclar directamente NaOCl con clorhexidina. Si el protocolo contempla distintos irrigantes, usa la secuencia y enjuagues indicados por tu institución.", "Avoid directly mixing NaOCl with chlorhexidine. If the protocol uses multiple irrigants, follow the institution's required sequence and intermediate rinses."))
                    }
                }
                item {
                    Card(colors = CardDefaults.cardColors(containerColor = Color(0xFFFFF2F2)), border = BorderStroke(1.dp, Color(0xFFD64545)), shape = RoundedCornerShape(16.dp)) {
                        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            Text("⚠ ${tr(lang, "Preparación / dilución", "Preparation / dilution")}", fontWeight = FontWeight.Black, color = Color(0xFF9A2D2D))
                            Text(tr(lang, "Por seguridad, la app no da una receta para fabricar o diluir hipoclorito a partir de productos domésticos o concentraciones desconocidas. Usa irrigante dental comercial o preparación institucional validada y verifica etiqueta, concentración, fecha y almacenamiento.", "For safety, the app does not provide a recipe for making or diluting hypochlorite from household products or unknown stock. Use a commercial dental irrigant or a validated institutional preparation and verify label, concentration, date and storage."))
                        }
                    }
                }
                item {
...
private fun defaultCanals(tooth: Int): List<EndoCanalRow> {
    ...
    return when {
        quadrant >= 5 && unit in listOf(4, 5) -> listOf(EndoCanalRow("C1"), EndoCanalRow("C2"), EndoCanalRow("C3"))
        ...
        unit in listOf(6, 7, 8) && quadrant in listOf(1, 2) -> listOf(EndoCanalRow("MV1"), EndoCanalRow("MV2"), EndoCanalRow("DV"), EndoCanalRow("P"))
...
private fun treatmentOrientation(dentition: String, session: EducationalSession, lang: String): String {
    ...
    return when (dentition) {
        ...
        "immature" -> if (a.sensitivityNegative) tr(lang, "Necrosis con desarrollo radicular incompleto: valorar endodoncia regenerativa, apicoformación u otra estrategia especializada.", "Necrosis with incomplete root development: assess regenerative endodontics, apexification or another specialized strategy.") else tr(lang, "Priorizar conservación de pulpa vital para favorecer desarrollo radicular cuando el caso lo permita.", "Prioritize vital-pulp preservation to support root development when the case permits.")
        else -> when {
...
private fun finalEndoWriting(lang: String, tooth: Int, pulpal: String, apical: String, procedure: String, canals: List<EndoCanalRow>, referencePoint: String, restoration: String, notes: String): String {
    ...
    return if (lang == "en") "Tooth $tooth. Pulpal diagnosis: $pulpal. Apical diagnosis: $apical. Procedure: $procedure. Coronal reference: $ref. Working lengths: $wl. Irrigation: according to institutional endodontic protocol, with rubber-dam isolation and extrusion control. Coronal restoration/seal: $rest. Instructions/follow-up: $follow."
    else "OD $tooth. Diagnóstico pulpar: $pulpal. Diagnóstico periapical: $apical. Procedimiento: $procedure. Referencia coronal: $ref. Longitudes de trabajo: $wl. Irrigación: conforme al protocolo endodóntico institucional, con aislamiento absoluto y control de extrusión. Restauración/sellado coronal: $rest. Indicaciones/seguimiento: $follow."
}

private fun numericLength(raw: String): String = raw.filter { it.isDigit() || it == '.' || it == ',' }.replace(',', '.').take(5)

private fun List<EndoCanalRow>.updated(index: Int, value: EndoCanalRow): List<EndoCanalRow> = mapIndexed { i, row -> if (i == index) value else row }
